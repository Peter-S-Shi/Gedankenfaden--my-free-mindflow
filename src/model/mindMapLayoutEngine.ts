...
function makeSizeOf(nodes: CanonicalNode[]): (id: string) => { width: number; height: number } {
  ...
  return (id: string) => {
    ...
    // Hardening: Persistent Manual Node Sizing) takes priority.
    // When manualSize.width is absent, Auto Layout recomputes dynamic
    // text-first width with single-line bias and 360px ceiling.
    const declaredWidth = node.manualSize?.width;
